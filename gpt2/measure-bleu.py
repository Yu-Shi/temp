...
def calc_nltk_bleu(args):
  ...
  with open(args.prediction_file, 'r', encoding='UTF-8') as f:
    ...
    for line in f:
      ...
      if args.reversed:
        unchanged_bleu += sentence_bleu([raw], target, weights=(0, 1, 0, 0), smoothing_function=chencherry.method1)
        output_bleu += sentence_bleu([raw], output, weights=(0, 1, 0, 0), smoothing_function=chencherry.method1)
      else:
        unchanged_bleu += sentence_bleu([target], raw, weights=(0, 1, 0, 0), smoothing_function=chencherry.method1)
        output_bleu += sentence_bleu([target], output, weights=(0, 1, 0, 0), smoothing_function=chencherry.method1)
      ln += 1
...
def main():
  ...
  parser.add_argument("--prediction_file", default=None, type=str, required=True, help="NDJson")
